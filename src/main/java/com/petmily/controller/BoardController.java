...
public class BoardController {
	...
	public ModelAndView protectDetail(@RequestParam int board_idx, HttpSession session) { // board_idx값을 이용하여 해당 게시판 글 추출
	...
	//소현: 나의 모금리스트페이지(마이페이지)
		...
		public @ResponseBody HashMap<String, Object> getmyList(@RequestParam HashMap<String, String> params,HttpSession session) {
	   ...
	   // 소현: 글 등록
			...
			public @ResponseBody HashMap<String, Integer> 
				fileDel(@RequestParam("fileName") String fileName
						, HttpSession session) {
			...
			public @ResponseBody HashMap<String, Integer> 
			filetwoDel(@RequestParam("fileName") String fileName, HttpSession session) {
				logger.info("file delete 요청");
				String root = session.getServletContext().getRealPath("/");		
