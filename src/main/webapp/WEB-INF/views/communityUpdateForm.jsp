...
<html>
	...
	<body>
		...
		<form id="communityUpdate" action="communityUpdate" method="post">
			<div id="conDiv">
				<h1>커뮤니티 수정</h1>
				<input type="hidden" name="board_idx" value="${cUpdateForm.board_idx}" />
				<table id="detailTable" style="border:1px solid gray">
				<tr class = "littleTr">
					<th style="width:100px">작성자</th><td class="littleTd" style="width:500px">${cUpdateForm.board_writer}</td>
					<%-- <td><input id="board_writer" type="text" name="board_writer" readOnly value="${sessionScope.loginId}"/></td> --%>
					<th style="width:100px">작성일</th><td>${cUpdateForm.board_regDate}</td>
				</tr>
				<tr class = "littleTr">
					<th style="width:100px">제　목</th>
					<td class="littleTd" style="width:500px">
						<input type="text" name="board_title" value="${cUpdateForm.board_title}"
						onkeyup="chkword(this, 70)"/></td>
					<th style="width:100px">조회수</th><td>${cUpdateForm.board_hit}</td>
				</tr>
				<tr>
			...
			<table id="uploadTable">
					<tr>
						<td>사 진 첨 부  
						<input type="button" onclick="cFileUp()" value="첨부"/></td>
					</tr>
					<tr>
	...
	<script>
		...
		function cFileUp() {
			//fileUpload 새 창을 띄운다.
			var myWin = window.open("./cFileUploadForm", "파일 업로드",	"width=400, height=100");
		}
		//수정 요청
