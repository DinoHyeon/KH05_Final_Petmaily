	...
	<body>
		...
		<div id="conDiv">
			...
			<table id="selectTable">
				<tr><td><select	id="cSearchKey" name="cSearchKey">
							<option value="검색 키워드" selected>검색 키워드</option>
							<option value="제목">제목</option>
							<option value="작성자">작성자</option>
							<option value="내용">내용</option>
						</select></td>
				<td id="valTd"><input type="text" id="cSearchVal" name="cSearchVal" /></td>
				<td><input type="button" id="searchBtn" onclick="communityList(showPageNum)" value="검색" /></td></tr>
			</table>
			<table id="listTable">
				<thead>
					<tr>
			...
			<input type="button" id="write" value="글 작성" onclick="location.href='./communityForm'"/>
			<div id="paging"></div>
		</div>
	</body>
	<script>
			...
			data.cList
					.forEach(function(item) {
						...
						content += "<td><a href='./communityDetail?board_idx="
								+ item.board_idx + "'>" + item.board_title
								+ "</td>";
						content += "<td>" + item.board_writer + "</td>";
						content += "<td>" + date.toLocaleDateString("en-KOR")
								+ "</td>";
